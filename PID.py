# Paul Takhar Joshua Rossie Jonathan Chesney Kent Gleim
...
#initilize timers and set their pin number as well as frequency
...
#in order words we want the line to be in the center of the screen
setpoint=80;
...
#i keep leds on to helpo illuminate the track
blue_led.on()
green_led.on()
red_led.on()
...
while(run <1):
    ...
    #we will use this "count" variable to keep track of how many blobs are on the screen
    count = 0;

    #this adds up the number of blobs in our region of interest, this is important so that we can idetify the finish line (3 blobs) and main track (1 blob) and no track (0 blobs)
    for i in blobs:
        count += 1
    ...
    #not needed but just tells us the error value
    #print("error is ")
    #print(error1)
